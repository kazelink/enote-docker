...
const BLOCK_ALIGN_TAGS = new Set(['p', 'div', 'h1', 'h2', 'blockquote']);
const INDENT_TAGS = new Set(['p', 'div', 'h1', 'h2', 'blockquote']);
...
function sanitizeClass(tag, value) {
    ...
    if (tag === 'ul') {
        return classes.includes('checklist') ? 'checklist' : '';
    }
    if (tag === 'li') {
        return classes.includes('checked') ? 'checked' : '';
    }
    return '';
}
